...
def get_ground_truth_answers() -> dict:
    """
    Эталонные ответы на основе содержимого data/docs.txt.
    
    Returns:
        словарь с эталонными ответами для каждого вопроса
    """
    ground_truth = {
        "Что такое машинное обучение?": 
        ...
        "Какие основные типы машинного обучения существуют?":
            "Основные типы машинного обучения включают обучение с учителем, обучение без учителя и обучение с подкреплением. Обучение с учителем использует размеченные данные для предсказания результатов.",
        
        "Что такое нейронная сеть?":
            "Нейронные сети - это модели машинного обучения, вдохновлённые структурой человеческого мозга. Они состоят из слоёв взаимосвязанных нейронов, которые обрабатывают информацию. Глубокие нейронные сети содержат множество скрытых слоёв и способны обучаться сложным паттернам.",
...
def prepare_dataset(pipeline: RAGPipeline, questions: list) -> Dataset:
    ...
    for i, question in enumerate(questions, 1):
        ...
        # Ground truth - эталонный ответ из нашей базы знаний
        ground_truth = ground_truth_answers.get(question, "Эталонный ответ не найден")
        ground_truths_list.append(ground_truth)
        
        print(f"     [+] Ответ получен от OpenAI API")
        print(f"     [+] Эталонный ответ: {ground_truth[:100]}...")
...
def evaluate_rag_system():
    ...
    print("   (используем эталонные ответы из базы знаний)")
    print("   (это займёт 2-3 минуты, так как RAGAS использует OpenAI для оценки)\n")
    
    # Используем только стабильно работающие метрики
    print("   [+] Используем стабильные метрики RAGAS с ground truth\n")
...
ЭТАЛОННЫЕ ОТВЕТЫ:
  Используются ответы, извлечённые из файла data/docs.txt
  для более точной оценки качества системы.

ПРИМЕЧАНИЕ:
  Answer Relevancy отключена из-за несовместимости embeddings API
  в текущей версии RAGAS с langchain-openai.
  Для полной оценки используйте evaluate_simple.py
    """)
