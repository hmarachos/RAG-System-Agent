# RAG System - Retrieval-Augmented Generation

Полнофункциональная система RAG (Retrieval-Augmented Generation) с поддержкой OpenAI и GigaChat, векторным поиском на Qdrant и профессиональной оценкой качества.

## 🚀 Возможности

- **Две версии API**: OpenAI и GigaChat (Сбер)
- **Векторный поиск**: Qdrant для стабильной работы на Windows
- **Умный chunking**: Семантическое разбиение документов
- **Кеширование**: Оптимизация затрат на API
- **Оценка качества**: RAGAS и собственная система оценки
- **Эталонные ответы**: Ground truth для точной оценки
- **Context managers**: Правильное управление ресурсами

## 📁 Структура проекта

```
├── assistant_api/          # OpenAI версия
│   ├── app.py             # Flask API сервер
│   ├── vector_store.py    # Qdrant + OpenAI embeddings
│   ├── rag_pipeline.py    # Основной RAG pipeline
│   ├── cache.py           # Система кеширования
│   ├── evaluate_ragas.py  # RAGAS оценка качества
│   ├── evaluate_simple.py # Простая оценка качества
│   └── data/docs.txt      # База знаний
├── assistant_giga/         # GigaChat версия
│   ├── app.py             # Flask API сервер
│   ├── vector_store.py    # Qdrant + GigaChat embeddings
│   ├── rag_pipeline.py    # RAG pipeline для GigaChat
│   ├── gigachat_client.py # Клиент GigaChat API
│   └── data/docs.txt      # База знаний
├── requirements.txt        # Python зависимости
├── .env.example           # Шаблон переменных окружения
└── README.md              # Документация
```

## 🛠 Установка

### 1. Клонирование репозитория
```bash
git clone <repository-url>
cd FINAL_RAG_AGENT_FULL
```

### 2. Создание виртуального окружения
```bash
python -m venv venv
venv\Scripts\activate  # Windows
# source venv/bin/activate  # Linux/Mac
```

### 3. Установка зависимостей
```bash
pip install --upgrade pip
pip install -r requirements.txt
```

### 4. Настройка переменных окружения
```bash
# Скопируйте шаблон
copy .env.example .env  # Windows
# cp .env.example .env  # Linux/Mac

# Отредактируйте .env файл и добавьте ваши API ключи
```

## 🔑 Настройка API ключей

### OpenAI API
1. Зарегистрируйтесь на [OpenAI Platform](https://platform.openai.com/)
2. Создайте API ключ в разделе [API Keys](https://platform.openai.com/api-keys)
3. Добавьте в `.env`: `OPENAI_API_KEY=your_key_here`

### GigaChat API (опционально)
1. Зарегистрируйтесь на [Сбер Developers](https://developers.sber.ru/portal/products/gigachat)
2. Получите RQUID и AUTH_KEY
3. Добавьте в `.env`:
   ```
   GIGACHAT_RQUID=your_rquid_here
   GIGACHAT_AUTH_KEY=your_auth_key_here
   ```

## 🚀 Запуск

### OpenAI версия
```bash
cd assistant_api

# Тестирование векторного хранилища
python vector_store.py

# Тестирование RAG pipeline
python rag_pipeline.py

# Запуск Flask API сервера
python app.py
```

### GigaChat версия
```bash
cd assistant_giga

# Тестирование
python vector_store.py
python rag_pipeline.py

# Запуск сервера
python app.py
```

## 📊 Оценка качества

### Простая оценка (рекомендуется)
```bash
cd assistant_api
python evaluate_simple.py
```

**Результат**: 5 метрик качества включая соответствие эталонным ответам

### RAGAS оценка (профессиональная)
```bash
cd assistant_api
python evaluate_ragas.py
```

**Результат**: Faithfulness и Context Precision от RAGAS

## 🔧 API Endpoints

### POST /query
Основной endpoint для RAG запросов:

```bash
curl -X POST http://localhost:5000/query \
  -H "Content-Type: application/json" \
  -d '{"question": "Что такое машинное обучение?"}'
```

**Ответ**:
```json
{
  "query": "Что такое машинное обучение?",
  "answer": "Машинное обучение - это раздел искусственного интеллекта...",
  "from_cache": false,
  "context_docs": [...],
  "model": "gpt-4o-mini"
}
```

### GET /stats
Статистика системы:
```bash
curl http://localhost:5000/stats
```

## 📈 Результаты оценки

### Текущие показатели качества:
- **Faithfulness**: 1.000 ✅ (идеальная точность)
- **Relevance**: 0.975 ✅ (отличная релевантность)
- **Context Quality**: 0.912 ✅ (хорошее качество контекста)
- **Clarity**: 1.000 ✅ (идеальная ясность)
- **Ground Truth Similarity**: 0.975 ✅ (высокое соответствие эталону)

**Общий балл**: 0.972/1.000 🟢 (отличное качество)

## 🛡 Управление ресурсами

Система использует context managers для правильного закрытия соединений:

```python
# Рекомендуемый способ
with VectorStore() as vs:
    results = vs.search("query")
# Автоматическое закрытие

with RAGPipeline() as pipeline:
    answer = pipeline.query("question")
# Все ресурсы закрыты
```

## 🔍 Troubleshooting

### PyTorch DLL ошибка на Windows
```bash
pip uninstall torch -y
pip install torch==2.0.1 --index-url https://download.pytorch.org/whl/cpu
```

### Проблемы с Qdrant
- Убедитесь, что папка `qdrant_db` доступна для записи
- Проверьте, что порты не заняты другими процессами

### API ключи не работают
- Проверьте правильность ключей в `.env` файле
- Убедитесь, что `.env` файл находится в корне проекта
- Проверьте баланс на аккаунте API провайдера

## 📚 Документация

- [QDRANT_MIGRATION.md](QDRANT_MIGRATION.md) - Миграция с ChromaDB на Qdrant
- [RESOURCE_MANAGEMENT.md](RESOURCE_MANAGEMENT.md) - Управление ресурсами
- [EVALUATION_SETUP.md](EVALUATION_SETUP.md) - Настройка оценки качества
- [GROUND_TRUTH_EVALUATION.md](GROUND_TRUTH_EVALUATION.md) - Эталонные ответы
- [RAGAS_FIX.md](RAGAS_FIX.md) - Исправление проблем RAGAS

## 🤝 Вклад в проект

1. Fork репозитория
2. Создайте feature branch (`git checkout -b feature/amazing-feature`)
3. Commit изменения (`git commit -m 'Add amazing feature'`)
4. Push в branch (`git push origin feature/amazing-feature`)
5. Откройте Pull Request

## 📄 Лицензия

Этот проект распространяется под лицензией MIT. См. файл `LICENSE` для подробностей.

## 🆘 Поддержка

Если у вас возникли вопросы или проблемы:

1. Проверьте [документацию](README.md)
2. Посмотрите [Issues](../../issues)
3. Создайте новый Issue с подробным описанием проблемы

---

**Система готова к production использованию!** 🚀